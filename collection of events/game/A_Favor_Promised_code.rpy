label A_Favor_Promised :

    scene mint AFP 01
    with long_fade
    pause(1.0)

    scene mint AFP 00
    with long_dissolve
    pause(1.5)

    scene mint AFP 01
    with long_dissolve
    pause(1.0)

    scene mint AFP 02
    with medium_fade
    Daniel "{i}Arg ..."
    Daniel "{i}This sucks , {w}how are there this 
    many channels and nothing to watch ?"
    Daniel "{i}I miss my old shows"

    scene mint AFP 03
    with dissolve
    Daniel "{i}Fuck it , {w}I give up"
    Daniel "{i}Let's go fuck with Katie"

    scene mint AFP 04
    with medium_fade
    Daniel "{i}Hmm ... {w}awfully quiet"
    Daniel "{i}I wonder if she's sleeping?"
    Daniel "{i}Well, {i}if she is ... 
    {w}then this is about to be funny as hell"

    scene mint AFP 05
    with fade_To_Instant
    with vpunch
    Daniel "HEY KATIE !"
    Daniel "YOU SLEEPING !?"

    scene mint AFP 06
    with vpunch
    Katie "{i}{b}* Scream *"
    Daniel "{i}Guess not"

    scene mint AFP 07
    with fade
    Katie "Daniel , {w}what the hell"
    Katie "Why'd you do that ?"
    Katie "I almost peed myself"
    Daniel "Huh ... {w}do what ?"
    Katie "Arg ... {w}I swear"

    scene mint AFP 08
    with medium_fade
    Daniel "So ... {w}whatcha doin'"
    Katie "Putting on an earring ..."
    Katie "What do you want ?"
    Daniel "I was bored , {w}so I thought ..."
    Daniel "Let's go see what Katie's up to"
    Katie "Ugh ... {w}go away"
    Daniel "Why ?"
    Daniel "Are you doing something ?"
    Daniel "Maybe going somewhere ?"

    scene mint AFP 09
    with dissolve
    Katie "Just going to the mall to hang out with some friends"
    Daniel "The ... {w=1}mall ... {w=1}Really ?"
    Katie "Yup"
    Daniel "At night time ?"
    Katie "Yah"

    scene mint AFP 10
    with fade
    Daniel "{i}She's such a shitty liar"
    Daniel "{i}No ... {w}scratch that"
    Daniel "{i}That's disrespectful to the shitty liars"

    scene mint AFP 11
    with fade
    Daniel "{b}{i}* Sigh *{plain} ... Okay , 
    {w}I'll just mention this to the old man when he gets home from work"
    Daniel "I'm sure he won't find that suspicious or anything ..."
    Daniel "Strange coincidence that you decided to go to 
    the mall tonight though"
    Daniel "The same night that huge party up in Vinewood Hills happens to be on"

    scene mint AFP 12
    with medium_fade
    Katie "Daniel ... {w=1}wait !"
    Daniel "What , {w}I'm gonna go play video games in my room Katie"
    Daniel "What do you want ?"

    scene mint AFP 13
    with fade
    Katie "I ... {w}um ... {w}might have not been telling the truth"
    Daniel "{i}{b}* Gasp *{plain} ... Katie"
    Daniel "You tricked me"
    Katie "I know , {w}I'm sorry"
    Katie "I just really wanted to go"

    scene mint AFP 14
    with fade
    Katie "Please Daniel , {w}cover for me tonight ... {size=24}{cps=5}{w}please"
    Daniel "I don't know , {w}you fooled me once"
    Daniel "Fool me twice and it's shame on me"

    scene mint AFP 15
    with fade
    Katie "No"
    Katie "I swear , {w}I won't lie to you anymore Daniel"
    Katie "Please ... {cps=5}{w=1}PLEEEAASSE{/cps} {w=0.7}Daniel"

    scene mint AFP 16
    with fade
    Daniel "ARG ... {w}fine"
    Daniel "You're lucky you're my favorite"
    Daniel "But you owe me a favor, {w}BIG TIME"
    Katie "I will , {w}thanks Daniel"
    Daniel "Yah ... {w=1}yah"
    Daniel "I'll add it to the pile"

    scene mint AFP 17
    with fade
    Daniel "Well , {w}since that's done ..."
    Katie "Wait Daniel"
    Daniel "What now ?"
    Katie "Can you help me with one more thing ?"
    Daniel "I'm not helping you paint your nails"
    Katie "No not that ... {w}just ... {w}come on"

    scene mint AFP 18
    with long_fade
    Daniel "Katie mind telling me what you're doing ?"

    scene mint AFP 19
    with fade
    Katie "I need to get something"
    Daniel "In the storage closet ?"
    Katie "Yup"
    Daniel "So then , {w}what do yah need me for ?"
    Katie "I need you to stand behind me and make sure I don't fall"
    Daniel "What ?!"
    Katie "It's on the top shelf"
    Daniel "Oh jeez"

    scene mint AFP 20
    with medium_fade
    Daniel "Hmmm"
    Daniel "Must be important for you to go through all this trouble"
    Katie "{size=24}Maybe"
    Daniel "What is it then ?"
    Katie "{size=24}I'll show you , {w}just wait"
    Daniel "{b}{i}* Sigh *"

    scene mint AFP 21
    with medium_fade
    Daniel "{i}This is so fucking stupid"
    Daniel "{i}Why do I get the feeling I'm going to regret helping"
    Daniel "{i}Maybe this is a bad idea"
    Daniel "{i}But , {w}I've already promised and I don't break promises"

    scene mint AFP 22
    with fade
    Daniel "{i}What could be so important that 
    she needs to hide it in the closet ?"
    Daniel "{i}I hope it's not drugs ... {w}I{nw=1.5}"

    scene mint AFP 23
    with fade
    pause

    scene mint AFP 24
    with fade
    Daniel "{i}Uhh ..."

    scene mint AFP 25
    with medium_fade
    Daniel "{i}Holy shit !"
    Daniel "{i}I just realized the situation"
    Daniel "{i}Oh fuck , {w}Katie's ass is litrally in my face right now"
    Daniel "{i}Oh fuck ... {w}shit"
    window hide
    pause
    Daniel "{i}I ... {w}I need to calm down"
    Daniel "{i}Remember , {w}this is Katie for fuck sakes"

    scene mint AFP 26
    with fade
    Katie "Daniel , {w}are you going to supporting me ?"
    Daniel "Uh , {w}I am"
    Katie "Really ?"
    Katie "It's cause , {w}I don't feel you holding on"
    Daniel "Oh , {w}I'm supposed to ?"
    Katie "Um ... {w}yeah, {w}that's kind of what support means"
    Daniel "Oh , {w}I thought you meant more like emotional support"
    Katie "What , {w}no ... {w}I want you to hold my legs"
    Daniel "Oh , {w}okay"

    scene mint AFP 27
    with medium_fade
    Daniel "{i}Oh fuck me"
    Daniel "{i}Okay , {w}it's fine"
    Daniel "{i}Just ... {w}don't look at her ass"

    scene mint AFP 28
    with medium_fade
    Daniel "{i}Okay"
    Daniel "{i}Now , {w}just grab her legs I guess"

    scene mint AFP 29
    with medium_fade
    Daniel "{i}Good god , {w}it looks even better at this angle"
    Daniel "{i}Fuck , {w}why is such a nice ass attached to Katie of all people"

    scene mint AFP 30
    with medium_fade
    Daniel "{i}Jeez , {w}her leggings are soo soft"
    Daniel "{i}This is so wrong , {w}but damn I can't help it"
    Daniel "{i}I'm starting to get a chub"
    Daniel "{i}If Katie wasn't ... {w}fuck"

    scene mint AFP 31
    with fade
    Daniel "{i}Erg fuck , {w}what am I thinking right now"
    Daniel "{i}Fuck this is wrong , {w}but I can't help it"
    Daniel "{i}And she's just happily searching away, 
    {w}paying absolutely no mind to me"
    Daniel "{i}Probably because of how much she trusts me"

    scene mint AFP 32
    with medium_fade
    Daniel "{i}Maybe ... {w}maybe I can just ..."
    Daniel "{i}No Daniel , {w}don't let you mind go there"
    window hide
    pause(2.0)
    Daniel "{i}I ... {w} I just have to"
    Daniel "{i}Just real quick"
    Daniel "{i}Just this one time , {w}then ..."
    Daniel "{i}I'm seeing a shrink"

    scene mint AFP 33
    with dissolve
    pause

    scene mint AFP 34
    with dissolve
    Daniel "{i}slowly ..."

    scene mint AFP 35
    with fade
    Daniel "{i}That's it ... {w}just keep focusing on what you're doing"

    scene mint AFP 36
    with fade
    Daniel "{i}sorry Katie"

    scene mint AFP 37
    with dissolve
    window hide
    pause(1.0)
    Daniel "{i}Just real quick"

    scene mint AFP 38
    with medium_fade
    Daniel "{i}{b}{size=24}* sniff *"

    scene mint AFP 39
    with dissolve
    Daniel "{i}{b}{size=28}{cps=5}* Sniffff *"
    Daniel "{i}{w=1}{cps=5}fuckk"

    scene mint AFP 40 start
    with long_fade
    pause(1.0)
    scene mint AFT 40
    pause